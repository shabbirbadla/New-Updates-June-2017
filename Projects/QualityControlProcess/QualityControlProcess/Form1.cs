...
namespace QualityControlProcess
{
    public partial class Form1 : Form
    {
        ...
        private void ButtonState(bool New, bool Edit, bool Save, bool Cancel, bool Delete, bool Copy)//<--- Added By Pankaj On 26/04/2014 Delete and Copy parameter added for Bug-22827
        {
            ...
            //<--- Added By Pankaj On 26/04/2014 for Bug-22827 Start //wwww
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (dgvParam.Rows.Count ==1  )
            {
                ...
                MessageBox.Show("Please Enter Goods details", this._ApplText, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dgvParam.Focus();   
                return;
            }
        ...
        private void btnCancel_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Discard Changes ?", this._ApplText, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (IsNoRec())
                    ...
                    ButtonState(true, true, false, false, true, true);
                isCancel = false;
                dgvParam.Refresh(); //added by Ruchit on 02-05/2017
            }
        }
        ...
        private void MinedgvParam_KeyPress(object sender, KeyPressEventArgs e)
        {
             ...
             if (CurrentMode == Mode.Add | CurrentMode == Mode.Edit)
             {
                 ...
                 if (dgvParam.CurrentCell.OwningColumn.HeaderText == "Standard Value")
                 {
                     ...
                     {
                        //if ((e.KeyChar >= 48 && e.KeyChar <= 57) || e.KeyChar == 8 || e.KeyChar == 45) Commented By Kishor A. for Bug-26665 on 12/08/2015
                        if ((e.KeyChar >= 48 && e.KeyChar <= 57) || e.KeyChar == 8 || e.KeyChar == 45 || e.KeyChar == '.') //Added By Kishor A. for Bug-26665 on 12/08/2015
                        {
                            e.Handled = false;
                            //Added By Kishor A. for Bug - 26665 on 12 / 08 / 2015 Start..
                            if (e.KeyChar == '.' && (sender as TextBox).Text.IndexOf('.') > -1)
                            {
                                e.Handled = true;
                            }
                            //Added By Kishor A. for Bug - 26665 on 12 / 08 / 2015 End...
                        }
                        else
                            e.Handled = true;
                        return;
                 ...
                 {
                    //if ((!String.IsNullOrEmpty(dgvParam["StandardValue", dgvParam.CurrentCell.RowIndex].Value.ToString()) && (e.KeyChar >= 48 && e.KeyChar <= 57)) || e.KeyChar == 8) //Commented By Kishor A. for Bug-26665 on 12/08/2015
                    if ((!String.IsNullOrEmpty(dgvParam["StandardValue", dgvParam.CurrentCell.RowIndex].Value.ToString()) && (e.KeyChar >= 48 && e.KeyChar <= 57)) || e.KeyChar == 8 || e.KeyChar == '.')  //Added By Kishor A. for Bug-26665 on 12/08/2015
                    {
                        e.Handled = false;
                        //Added By Kishor A. for Bug - 26665 on 12 / 08 / 2015 Start..
                        if (e.KeyChar == '.' && (sender as TextBox).Text.IndexOf('.') > -1)
